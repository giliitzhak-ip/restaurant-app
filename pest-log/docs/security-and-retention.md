...
שש הטבלאות של מסלול העבודה (`route_templates`, `maintenance_routes`,
`route_assignments`, `route_visits`, `visit_focus_items`,
`visit_status_history`) מוגנות באותו מודל: RLS עם `FORCE` על כולן, והפרדה
מלאה בין ארגונים. בנוסף:

- **עובד רואה ומעדכן רק מסלולים שהוקצו לו** — ישירות או דרך
  `route_assignments`. מנהל רואה את כל מסלולי הארגון.
- **יצירת מסלול, שיוך עובדים, מחיקת מסלול או תבנית** — מנהל בלבד.
- **היסטוריית הביקורים היא append-only**: אין עדכון ואין מחיקה, גם לא
  למנהל ולא ל-superuser. החריג היחיד הוא מחיקת מסלול שלם על ידי מנהל,
  שבה השורות נמחקות במפל אחרי שהמסלול עצמו נמחק.
- **ביקור שכבר התחיל אינו ניתן למחיקה** — אפשר רק לבטל אותו עם סיבה,
  והביטול נרשם. הסרת לקוח מהמסלול אינה נוגעת במאגר הלקוחות.
- **הערות פנימיות לצוות** (`route_visits.internal_notes` ודגש שסומן
  `is_internal`) אינן מוצגות ללקוח ואינן נכנסות ל-PDF של היומן. סכמת
  היומן משמיטה כל שדה שאינו חלק מהמסמך הרשמי, ויש בדיקה שמוודאת זאת.
- **מיקום המדביר** נרשם רק אם המשתמש אישר גישה למיקום במכשיר, ולעולם
  אינו מעכב את סימון ההגעה או הסיום.
- **audit**: שינוי סדר, יצירת מסלול מתבנית ושינויי סטטוס משמעותיים
  נרשמים ב-`audit_events` — בלי שמות לקוחות ובלי כתובות.
- כתיבה ל-`audit_events` מהפונקציות של המסלול עוברת דרך
  `app.write_audit_event`, שמוודאת שהאירוע שייך לארגון של המשתמש.

הבדיקות: `tests/rls/routes.test.ts` (17 בדיקות) ו-`tests/integration/routes.test.ts`
(15 בדיקות) מול Postgres אמיתי.
