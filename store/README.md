...
| `npm run simulate` | סימולטור הזמנות ותפעול (ראו בהמשך) |
...
## סימולטור הזמנות ותפעול

`Admin ← סימולטור`, או מה-CLI:

```bash
npm run simulate -- --count 40 --days 30              # תערובת תרחישים ריאלית
npm run simulate -- --scenarios HAPPY_PATH,REFUNDED   # תרחישים ספציפיים
npm run simulate -- --seed 42                         # ריצה שניתן לשחזר
npm run simulate -- --purge                           # מחיקת כל נתוני הסימולציה
```

מייצר פעילות דמה שעוברת **באותו קוד בדיוק** שלקוח אמיתי עובר בו — `placeOrder`,
`applyPaymentResult`, `transitionOrder`, `refundOrder` — ולכן ריצה גם מוכיחה
ששריון המלאי, האידמפוטנטיות וחוקי הסטטוס עובדים.

תרחישים: הזמנה שהושלמה, הזמנה בטיפול, ממתינה לתשלום, תשלום שנכשל, ביטול,
זיכוי (מלא או חלקי), עגלה נטושה, וניסיון הזמנה מעבר למלאי (מוודא שהמערכת דוחה).

בטיחות:

* כל הזמנה ולקוח מסומנים `isSimulated`, ומוצגים עם תג ״סימולציה״ בהזמנות ובדשבורד.
* הדשבורד מאפשר לעבור לתצוגת ״נתונים אמיתיים בלבד״ בלחיצה.
* המחיקה מסירה רק רשומות מסומנות ומחזירה את המלאי שנצרך — הזמנות אמיתיות
  אינן נוגעות, וזה מכוסה בבדיקה ייעודית.
* בסביבת Production הסימולטור מושבת אלא אם הוגדר `ENABLE_SIMULATOR=true`.
* לא נשלחים מיילים בזמן סימולציה, והזהויות שנוצרות סינתטיות לחלוטין.
...
במקביליות, webhook כפול, זיכוי, העלאת תמונות מקצה לקצה, הגנת /admin, אכיפת
הפרסום הרגולטורי, ותרחישי הסימולטור — כולל הוכחה שהמחיקה מחזירה מלאי במדויק
ואינה נוגעת בהזמנות אמיתיות.
