...
| Verification | Mostly `VERIFIED`, some `PENDING`, a few `REJECTED`/`SUSPENDED`. **No documents back it.** The seeder does not create licence or insurance rows, because a fabricated licence is exactly the thing `provider_documents` exists to prevent — so `provider_missing_documents` reports an outstanding licence for most of the network, and says so honestly. The verification gate applies when an admin verifies somebody, so these synthetic verifications stand; they are not evidence of anything and are tagged `is_demo`. The expiry sweep deliberately does not touch them either: it asks "whose approval expired", not "whose documents are missing" (D-029). |
| Ratings | **Derived, not asserted.** The seeder writes a completed job, its offer, its assignment and a review behind every counted review — 14,757 jobs and 10,908 reviews — and then computes `rating_avg`, `rating_count` and `completed_jobs` from those rows. Counts are capped at 24, so the network reads as a young marketplace rather than an established one; that is the price of the numbers being true. Four validator checks assert the aggregate still equals its detail. See [DECISIONS D-032](DECISIONS.md). |
| Review text | None. The ratings are real rows; the comments would be invented testimonials, which is the most persuasive kind of fake content. The profile says "no written reviews yet" and means it. |
