-- ===========================================================================
-- 0019 — Filter by geography before evaluating availability
--
-- 0018 applied provider_is_available_at() inside the candidate CTE and the
-- distance filter in the outer query. Postgres therefore evaluated the
-- expensive per-provider availability function — which itself reads settings,
-- overrides, weekly rules and job assignments — for EVERY provider in the
-- category, then threw most of them away on distance.
--
-- Measured at 1024 providers: a request in the middle of the Mediterranean
-- with zero candidates still cost ~62ms, because ~250 plumbers had their
-- availability computed first.
--
-- Cheap, index-backed predicates now run first (category, verification,
-- spatial ST_DWithin against the GiST index), and availability is evaluated
-- only for providers who are geographically plausible.
-- ===========================================================================
    ...
    select r.*
     ...
     where not exists (
             select 1 from public.job_offers o
              where o.job_id = r.job_id and o.provider_id = r.provider_id
           )
